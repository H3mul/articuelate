...
//! Collapsing is handled by the assembler via `dyn_container`.

use floem::reactive::{
    create_get_update, create_rw_signal, create_signal, RwSignal, SignalGet, SignalUpdate,
    SignalWith,
};
use floem::unit::Pct;
use floem::views::{
    button, empty, h_stack, label, slider, tab, text, text_input, v_stack, Decorators,
};
use floem::IntoView;
...
pub fn view(selected: RwSignal<usize>, cues: RwSignal<im::Vector<Cue>>) -> impl IntoView {
    ...
    let duration_pct =
        create_get_update(duration, |v: &f64| Pct(v / 10.0 * 100.0), |p: &Pct| {
            p.0 / 100.0 * 10.0
        });
    let volume_pct = create_get_update(volume, |v: &f64| Pct((v + 60.0) / 60.0 * 100.0), |p: &Pct| {
        p.0 / 100.0 * 60.0 - 60.0
    });
    ...
    .style(|s| s.flex_col().flex_grow(1.0).min_height(0.0).padding(14.0).background(BG));

    v_stack((tab_buttons, content))
        .style(|s| s.flex_col().height(240.0).background(BG).border_top(1.0).border_color(BORDER))
}
...
fn tab_button(
    ...
    button(name)
        .action(move || st.set(i))
        .style(move |s| {
            s.padding_horiz(14.0)
                .padding_vert(8.0)
                .font_size(12.0)
                .color(if at.get() == i { TEXT } else { TEXT_DIM })
                .background(if at.get() == i { PANEL_ALT } else { PANEL })
                .apply_if(at.get() == i, |s| s.border_bottom(2.0).border_color(ACCENT))
...
fn general_tab(selected: RwSignal<usize>, cues: RwSignal<im::Vector<Cue>>) -> impl IntoView {
    ...
    let ctx = label(move || format!("Task {} — {}", s2.get() + 1, name_fn()))
        .style(|s| s.color(TEXT).font_size(13.0).font_weight(floem::text::Weight::BOLD));
...
fn routing_tab(routing: RwSignal<[[bool; 4]; 2]>) -> impl IntoView {
    ...
    v_stack((text("Crosspoint Matrix (Input → Outputs)").style(field_label()), matrix))
...
fn routing_row(r: usize, routing: RwSignal<[[bool; 4]; 2]>) -> impl IntoView {
    ...
    h_stack((
        text(["In L", "In R"][r]).style(|s| {
            s.font_family(MONO.to_string()).color(TEXT_DIM).font_size(11.0).width(44.0)
        }),
        cell_row,
    ))
    ...
    button(out_label)
        ...
        .style(move |s| {
            s.width(46.0)
                ...
                .font_size(12.0)
                .color(if routing.get()[r][c] { BG } else { TEXT_DIM })
                .background(if routing.get()[r][c] { ACCENT } else { PANEL_ALT })
                .border(1.0)
                .border_color(BORDER)
                .border_radius(4.0)
                .hover(|s| s.background(BORDER))
...
fn fades_tab(
    ...
    v_stack((dur_row, vol_row))
        .style(|s| s.flex_col().gap(14.0).margin_top(4.0))
}
...
fn field(label_text: &'static str, value: &'static str) -> impl IntoView {
    h_stack((
        ...
        text(value).style(|s| s.color(TEXT).font_family(MONO.to_string()).font_size(12.0)),
    ))
...
fn mono_value() -> impl Fn(floem::style::Style) -> floem::style::Style + 'static {
    move |s: floem::style::Style| s.color(ACCENT).font_family(MONO.to_string()).font_size(12.0).width(60.0)
}
