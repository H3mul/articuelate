...
use floem::reactive::{create_memo, Memo, RwSignal, SignalGet, SignalUpdate, SignalWith};
use floem::views::{
    container, h_stack, label, list, scroll, text, virtual_list, v_stack, Decorators,
...
pub fn view(
    ...
    v_stack((header, scroll(list).style(|s| s.width_full().flex_grow(1.0).min_height(0.0))))
        .style(|s| {
            s.flex_col()
                .flex_grow(1.0)
                .min_width(0.0)
...
fn cue_row(
    ...
    let follow_badge = match follow {
        ...
        other => text(other.badge()).style(|s| {
            s.color(LAPCE_BLUE)
                ...
                .font_size(9.0)
                .border(1.0)
                .border_color(LAPCE_BLUE.multiply_alpha(0.5))
                .border_radius(3.0)
                .padding_horiz(5.0)
                .padding_vert(1.0)
        }),
    ...
    let action_lines = list(
        actions
            .into_iter()
            .map(action_line)
            .collect::<Vec<_>>(),
    )
    .style(move |s| s.flex_col().gap(2.0).margin_left((depth as f64) * 14.0 + 34.0));
        ...
        .style(move |s| {
            s.width_full()
                ...
                .background(if is_selected() {
                    ACCENT_DIM.multiply_alpha(0.22)
                } else {
                    Color::TRANSPARENT
                })
                .border_bottom(1.0)
                .border_color(BORDER)
                .apply_if(act_c.get() == idx_c, |s| s.border_left(3.0).border_color(ACCENT))
        })
        .on_click(move |_| {
            let (s, i) = (sel, idx);
            s.set(i);
...
fn action_line(a: CueAction) -> impl IntoView {
    let CueAction { kind, target, detail } = a;
    ...
    let glyph_color = match kind {
        ActionKind::Play => ACCENT,
        ActionKind::Fade => LAPCE_BLUE,
        ActionKind::Stop => PANIC,
    };
    h_stack((
        text(glyph).style(move |s| s.color(glyph_color).font_size(10.0).min_width(14.0)),
        label(move || target.clone()).style(|s| s.color(TEXT).font_size(12.0)),
        label(move || detail.clone()).style(|s| s.color(TEXT_DIM).font_size(11.0).font_family(MONO.to_string())),
    ))
